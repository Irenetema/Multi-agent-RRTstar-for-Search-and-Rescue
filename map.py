...
class Map(Viewer):
    def __init__(self, top, down, left, right, refresh=True):
        ...
        self.obstacles = []  # fixed obstacles in the environment
        self.dynamic_obstacles = {}  # other agents in the environment
    ...
    def add_dynamic_obstacle(self, uav_id, obs):
        self.dynamic_obstacles[uav_id] = obs

    def remove_dynamic_obstacle(self, uav_id):
        # _ = self.dynamic_obstacles.pop(uav_id)
        self.dynamic_obstacles.pop(uav_id, None)

    def check_collision(self, other, avoidDist):
        for obs in self.obstacles:
            if obs.check_collision(other, avoidDist):
                return True
        # for dy_obs in self.dynamic_obstacles.keys():
        for key in self.dynamic_obstacles.keys():
            if self.dynamic_obstacles[key].check_collision(other, avoidDist):
                return True
        return False
